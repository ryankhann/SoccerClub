    ...
    <>
      ...
      <section className="relative h-screen w-full flex items-center justify-center text-center isolate overflow-hidden">
        ...
        <div className="max-w-[1200px] px-4 z-10 reveal" ref={addToReveals}>
          <h1 className="font-['Playfair_Display',serif] font-extrabold text-[clamp(2.5rem,9vw,5.2rem)] leading-[1.1] text-white">
            EU <span className="text-[#e6b422]">TRIALS</span>
          </h1>
          <div className="text-sm md:text-base uppercase tracking-[2px] my-4 backdrop-blur-sm text-white">EUROPEAN PRO OPPORTUNITIES</div>
        </div>
      </section>
      ...
      <div className="bg-gradient-to-br from-[#051f18] to-[#0b2c22] py-16 px-4">
        <div className="max-w-4xl mx-auto text-center reveal" ref={addToReveals}>
          <p className="text-white text-lg md:text-xl leading-relaxed">Direct entry trials with partner clubs across England, Spain, Germany, Italy, and the Netherlands. Selected players receive travel support and training stints with professional academies.</p>
        </div>
      </div>
      ...
      <div className="py-16 md:py-20 px-4 bg-[#1e2a32]">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto reveal" ref={addToReveals}>
          <div>
            <h2 className="text-2xl font-bold text-[#e6b422] mb-4">Partner Clubs Include</h2>
            <ul className="space-y-2 text-white/90">
              <li><i className="fas fa-futbol text-[#e6b422] mr-2"></i> Premier League Academies</li>
              <li><i className="fas fa-futbol text-[#e6b422] mr-2"></i> Bundesliga U19 Teams</li>
              <li><i className="fas fa-futbol text-[#e6b422] mr-2"></i> La Liga Youth Setups</li>
              <li><i className="fas fa-futbol text-[#e6b422] mr-2"></i> Eredivisie Development Programs</li>
            </ul>
          </div>
          <img className="w-full h-64 object-cover" src="/images/hero-1.jpg" alt="european stadium" onError={(e) => { e.target.style.display = 'none'; }} />
        </div>
      </div>
      ...
      <div className="py-16 px-4 bg-[#051f18]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {['hero-2.png', 'hero-3.jpg', 'hero-4.jpg'].map((img, idx) => (
            <img key={idx} className="w-full h-80 object-cover brightness-90 reveal" src={`/images/${img}`} alt="eu trials" ref={addToReveals} onError={(e) => { e.target.style.display = 'none'; }} />
          ))}
        </div>
      </div>
    </>
