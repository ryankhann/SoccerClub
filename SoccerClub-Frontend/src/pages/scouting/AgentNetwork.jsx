    ...
    <>
      ...
      <section className="relative h-screen w-full flex items-center justify-center text-center isolate overflow-hidden">
        ...
        <div className="max-w-[1200px] px-4 z-10 reveal" ref={addToReveals}>
          <h1 className="font-['Playfair_Display',serif] font-extrabold text-[clamp(2.5rem,9vw,5.2rem)] leading-[1.1] text-white">
            AGENT <span className="text-[#e6b422]">NETWORK</span>
          </h1>
          <div className="text-sm md:text-base uppercase tracking-[2px] my-4 backdrop-blur-sm text-white">LICENSED REPRESENTATION</div>
        </div>
      </section>
      ...
      <div className="bg-gradient-to-br from-[#051f18] to-[#0b2c22] py-16 px-4">
        <div className="max-w-4xl mx-auto text-center reveal" ref={addToReveals}>
          <p className="text-white text-lg md:text-xl leading-relaxed">Our exclusive agent network includes FIFA-licensed intermediaries who specialize in placing North American talent into European and MLS contracts. Players gain representation without upfront fees.</p>
        </div>
      </div>
      ...
      <div className="py-16 md:py-20 px-4 bg-[#1e2a32]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto text-center">
          {[
            { title: 'Contract Negotiation', desc: 'Expert handling of professional terms, bonuses, and protections.' },
            { title: 'Visa & Immigration', desc: 'Work permit assistance for international moves.' },
            { title: 'Career Planning', desc: 'Long-term strategic guidance through every transfer window.' },
          ].map((item, idx) => (
            <div key={idx} className="reveal p-6 bg-[#0c2b21] border-t-4 border-[#e6b422]" ref={addToReveals}>
              <i className="fas fa-handshake text-4xl text-[#e6b422] mb-3"></i>
      ...
      <div className="py-16 px-4 bg-[#051f18]">
        <div className="flex flex-wrap justify-center gap-6 max-w-6xl mx-auto reveal" ref={addToReveals}>
          {['Ronaldo.png', 'Mbappé.png', 'Messi.png'].map((img, idx) => (
            <img key={idx} className="w-32 h-32 rounded-full object-cover grayscale-[0.2]" src={`/images/${img}`} alt="agent headshot" onError={(e) => { e.target.style.display = 'none'; }} />
          ))}
        </div>
      </div>
    </>
