    ...
    <>
      ...
      <section className="relative h-screen w-full flex items-center justify-center text-center isolate overflow-hidden">
        ...
        <div className="max-w-[1200px] px-4 z-10 reveal" ref={addToReveals}>
          <h1 className="font-['Playfair_Display',serif] font-extrabold text-[clamp(2.5rem,9vw,5.2rem)] leading-[1.1] text-white">
            U17 <span className="text-[#e6b422]">NATIONAL</span>
          </h1>
          <div className="text-sm md:text-base uppercase tracking-[2px] my-4 backdrop-blur-sm text-white">PREMIER NATIONAL COMPETITION</div>
        </div>
      </section>
      ...
      <div className="bg-gradient-to-br from-[#051f18] to-[#0b2c22] py-16 px-4">
        <div className="max-w-4xl mx-auto text-center reveal" ref={addToReveals}>
          <p className="text-white text-lg md:text-xl leading-relaxed">Our U17 National Team competes at the highest level of youth soccer, facing MLS Next academies and international opponents. This squad features players with youth national team caps and professional academy offers.</p>
        </div>
      </div>
      ...
      <div className="py-16 md:py-20 px-4 bg-[#1e2a32]">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto reveal" ref={addToReveals}>
          <img className="w-full h-80 object-cover" src="/images/hero-3.jpg" alt="national team" onError={(e) => { e.target.style.display = 'none'; }} />
          <div>
            <h2 className="text-2xl font-bold text-[#e6b422] mb-4">Recent Achievements</h2>
            <ul className="space-y-3 text-white/90">
              <li><i className="fas fa-medal text-[#e6b422] mr-2"></i> 2024 National Cup Quarterfinalists</li>
              <li><i className="fas fa-user-graduate text-[#e6b422] mr-2"></i> 7 players called to USYNT camps</li>
              <li><i className="fas fa-plane text-[#e6b422] mr-2"></i> 2025 International Tour (Spain)</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="py-16 px-4 bg-[#051f18]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {['hero-1.jpg', 'hero-4.jpg', 'hero-2.png'].map((img, idx) => (
            <img key={idx} className="w-full h-80 object-cover brightness-90 reveal" src={`/images/${img}`} alt="match action" ref={addToReveals} onError={(e) => { e.target.style.display = 'none'; }} />
          ))}
        </div>
      </div>
    </>
